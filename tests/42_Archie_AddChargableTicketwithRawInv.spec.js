...
const locators = require('../Archie/AddChargableTicketwithRawInv/AddChargableTicketwithRawInv.json');
const fs = require('fs');
...
const {navigate_ticket,addticketwithRawInv,reports,SACcustomeraccledger } = require('../Archie/AddChargableTicketwithRawInv/AddChargableTicketwithRawInv.js');
...
test('Archie - Add Chargable Sevice ticket with Raw Inventory', async ({ page }) => {
    ...
    //Add Service Ticket With Raw Inventory
    await addticketwithRawInv(page,customerName,"RO","ASHOKBHAI",UpdateRaw_Inventory,"1" , "2000");
...
test('delete service ticket Chargable with Raw inventory',async ({ page }) => {
    console.log('Step 1: Login to Shree Aqua Care');
    await log_in(page, "Kishorbhai", "Testing", "ArchieEnterprice"); // called login function
    console.log('Step 2: Dashboard Page displayed');
   // await expect(page).toHaveURL('http://192.168.29.112:88/');
    await page.waitForTimeout(2000);
    await selectmenu(page, "Transaction");
    await deleteservice(page,"ChargableRaw",customerName,UpdateRaw_Inventory);

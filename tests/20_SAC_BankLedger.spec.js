...
const { verifyaddledger,verifypartialpayment,verifyduplicatebillno,VerifyReceiveAmt,addledgerforsale,outstandingReport, customeraccledger, VerifyConsistency, Bankledgerreport } = require('../Pages/BankLedgerAdd/BankLedger.js');
...
test('Bank Ledger Page', async ({ page }) => {
    ...
    await Search_User(page, null, "Vendor", null,vendorName );
    console.log('Step 6: Sucessfully Vendor Search');
...
test('Add BankLedger for sale & GST', async ({ page }) => {
...
test('Add BankLedger for Purchase', async ({ page }) => {
    ...
    await selectmenu(page, "Transaction"); 
    console.log('Step 4: Sucessfully Clicked on Transaction Menu');
    await addledgerforpurchase(page); 
...
test('Customer Account Ledger report for Vendor', async ({ page }) => {
    ...
    await selectmenu(page, "Reports"); 
    console.log('Step 4: Sucessfully Clicked on Reports Menu');
    await customeraccledger2(page); 
});

test('Verify Outstanding Reports and Customer Account Ledger consistency for Vendor', async ({ page }) => {
    ...
    await selectmenu(page, "Reports"); 
    console.log('Step 4: Sucessfully Clicked on Reports Menu');
    //await VerifyConsistency2(page); 
    await selectmenu(page, "Reports"); 
    await Bankledgerreport2(page);
});
